...
def multi_head_attention_forward(query,  # [tgt_len,batch_size, embed_dim]
    ...
    if is_print_shape:
        ...
        print("进入多头注意力计算:")
        print(
            f"\t 多头num_heads = {num_heads}, d_model={query.size(-1)}, "
            f"d_k = d_v = d_model/num_heads={query.size(-1) // num_heads}")
        ...
        print("\t" + "-" * 70)
        print("\t ***** 注意，这里的W_q, W_k, W_v是多个head同时进行计算的. 因此，"
              "Q,K,V分别也是包含了多个head的q,k,v堆叠起来的结果 *****")
