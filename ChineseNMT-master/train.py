...
def train(train_data, dev_data, model, model_par, criterion, optimizer):
    ...
    best_bleu_score = config.best_bleu_score
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        if bleu_score > best_bleu_score:
            model_path = config.model_path + str(
                epoch) + f"dev_loss_{dev_loss}_bleu_{bleu_score}" + ".pth"
            logging.info(f"数据保存地址{model_path}")
